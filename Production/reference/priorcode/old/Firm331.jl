...
function getylSIMD(yh::Float64, k::Float64, ph::Float64, pl::Float64, F::Firm)
  kern::Float64 = (k^(F.γ*F.α)/pl) - (ph/pl)* (yh/F.Θ[1])^F.α
  yl::Float64 = (kern ≥ 0.0) ? (kern)^(1/F.α)*F.Θ[2] : NaN
...
function fillRCol!(F::T, p::Matrix{Float64}, s::Int, c::Int)::T where T<:Firm
  ...
  @inbounds @simd for r ∈ 1:N #2x performance gain or more
  ...
  (kₜ₁::Float64, yₜ₁h::Float64) = F. ψ[r]
  (kₜ::Float64, yₜh::Float64) =  F.ψ[c]
  τₜ = F.τ[s]
...
function fillR!(F::T, p::Matrix{Float64})::T where T<:Firm
  N::Int = length(F.ψ)
  S::Int = length(F.Θ)

  @inbounds for s ∈ 1:S, c ∈ 1:N
    fillRCol!(F, p, s, c)
  end
...
function Firm(;  ζ::NTuple= DEF_ζ,
  ...
  ψ::Vector{NTuple} = combine2tuples(kstates, yhstates)
  N::Int = length(ψ)
  S::Int = length(Θ)
  ...
  #Mdisp::Matrix{Float64} = Matrix{Float64}(undef,N,N)
  #for r ∈ 1:N, c∈ 1:N
  #  Mdisp[r,c] = F.M[r,c,2][2]
  #end
