...
using DataFrames, Distributions, StatsBase, GZip, JLD, Gadfly, CTModCopy, DataSeries, Formatting
...
function writeTaxDataToDF()::Void
...
function scaleTaxData!(taxDFm, taxDFq, taxDFy)::NTuple{3,DataFrame}
    ...
    for df::DataFrame ∈  [taxDFm, taxDFq, taxDFy]
      for s ∈ [:corp_industrial_yield,
        ...
        if (s ∈ namesOfCols)
          checkVal::Float64 = abs(mean(dropna(df[:,s])))
...
function fuseDataSeries(series1::DataVector{T}, series2::DataVector{T};
  viaFunction::Function = pick2)::DataVector{T} where T<:Union{Float64,Int}

  #pre-allocate the output
  out::DataVector{T} = similar(series1)
  #loop through the series and take non-NA values where possible
  for i ∈ 1:length(out)
    if isna(series1[i])
      out[i] = series2[i]
    elseif isna(series2[i])
      out[i] = series1[i]
    else
...
fuseDataSeries(df::DataFrame, series1Sym::Symbol, series2Sym::Symbol; viaFunction::Function = pick2) =
  fuseDataSeries(df[:,series1Sym]::DataVector,df[:,series2Sym]::DataVector, viaFunction = viaFunction)::DataVector

function processYahooData!(taxDFm, taxDFq, taxDFy)::Void
...
function getMuniTaxRate(muniYield::DataVector{Float64}, riskFree::DataVector{Float64},
  creditSpread::DataVector{Float64})::DataVector{Float64}

  #pre-allocate
  taxRates::DataVector{Float64} = similar(muniYield)
...
getMuniTaxRate(df::DataFrame, muniYieldSymbol::Symbol, riskFreeSymbol::Symbol,
  creditSpreadSymbol::Symbol)::DataVector{Float64} =
...
function processMuniData!(taxDFm::DataFrame, taxDFq::DataFrame, taxDFy::DataFrame;
  ...
  taxDFy[:,:fred_spread_close] = fuseDataSeries(taxDFy[:,:corp_industrial_yield_close] .-
    taxDFy[:,:gfd_10y_gov_yield_close], taxDFy[:,:corp_t_note_spread_close], viaFunction=viaFunction)

  taxDFy[:,:fred_spread_avg] = fuseDataSeries(taxDFy[:,:corp_industrial_yield_avg] .-
  ...
  taxDFq[:,:fred_muni_yield_avg] =
    ...
    #combine the fred corporate yields and corporate spreads with the gfd treasury yields
  taxDFq[:,:fred_spread_close] = fuseDataSeries(taxDFq[:,:corp_industrial_yield_close] .-
    taxDFq[:,:gfd_10y_gov_yield_close], taxDFq[:,:corp_t_note_spread_close], viaFunction=viaFunction)

  taxDFq[:,:fred_spread_avg] = fuseDataSeries(taxDFq[:,:corp_industrial_yield_avg] .-
  ...
  taxDFm[:,:fred_muni_yield_avg] =
    ...
    #combine the fred corporate yields and corporate spreads with the gfd treasury yields
  taxDFm[:,:fred_spread_close] = fuseDataSeries(taxDFm[:,:corp_industrial_yield] .-
    taxDFm[:,:gfd_10y_gov_yield_close], taxDFm[:,:corp_t_note_spread], viaFunction=viaFunction)

  taxDFm[:,:fred_spread_avg] = fuseDataSeries(taxDFm[:,:corp_industrial_yield] .-
...
getCumexPersonalTax(egStats::DataVector{Float64}, capitalGains::DataVector{Float64})::DataVector{Float64} =
  1.0 .- egStats .* (1.0 .- capitalGains)

#helper method which uses symbols instead of datavectors
getCumexPersonalTax(df::DataFrame, egStatsSym::Symbol, capitalGainsSym::Symbol)::DataVector{Float64} =
  getCumexPersonalTax(df[:,egStatsSym], df[:,capitalGainsSym])

#this is a formulaic method for getting the personal tax rate from the capital gains rate
getCumexTotalTax(egStats::DataVector{Float64}, capitalGains::DataVector{Float64})::DataVector{Float64} =
  1.0 .- egStats

#helper method which uses symbols instead of datavectors
getCumexTotalTax(df::DataFrame, egStatsSym::Symbol, capitalGainsSym::Symbol)::DataVector{Float64} =
...
function getExPostFromIndex(curIndex::DataVector{Float64},
    periodsExPost::Int, periodsPerYear::Int = 1)::DataVector{Float64}

  N::Int = length(curIndex)
  outVec::DataVector{Float64} = similar(curIndex)
  outVec .= NA
...
function get3YSmoothedFromIndex(curIndex::DataVector{Float64},
    beginOffset::Int = - 2, endOffset::Int = 1, periodsPerYear::Int = 1)::DataVector{Float64}

  curIndexNoNA = dropna(curIndex)
  N::Int = length(curIndexNoNA)
  outVecNoNA::DataVector{Float64} = deepcopy(curIndexNoNA)
  outVecNoNA .= NA
  ...
  outVec::DataVector{Float64} = deepcopy(curIndex)
  outVec .= NA
  ctr = 1
...
function getRealizedFromIndex(curIndex::DataVector{Float64},
    periodsRealized::Int, periodsPerYear::Int = 1)::DataVector{Float64}
...
formPercentile(x::Float64, CDFFunc::Function, decimals::Int)::String = formNum(CDFFunc(x), decimals)

function writeSelectedDataTable(df::DataFrame;
      fields::Vector{Symbol}=[:munitax, :munitax0credit, :avgord, :topord, :tnote1, :tnote1r, :tnote1at, :tnote20, :tnote20r, :tnote20at],
  fieldNames::Vector{String}=["munitax", "0credit", "avgord", "topord", "Nominal", "Real", "Txd-Real", "Nominal", "Real", "Txd-Real"],
  decimals::Int = 1)
  ...
  #get the content
  rows::Vector{Vector{String}} = [Vector{String}(K) for r::Int ∈ 1:N]
  for r::Int ∈ 1:N
    for c::Int ∈ 1:K
      val::T where T<:Union{Float64,NAtype} = df[r, fields[c]]
      rows[r][c] = isna(val) ? "" : "$(formNum(val*100, decimals, scaleHurdle=9999.))"
    end
  end
  ...
  writeTables2File([tableText], path=OUTPUT_PATH, HEADER_NAME, FOOTER_NAME,
...
function writeSummaryTable(df::DataFrame, fields::Vector{Symbol}, fieldNames::Vector{String};
  ...
  decimals::Int=3, σsAbove2016::Float64 = 2.0)::String
  ...
  summaryColNames = [summaryColNames; ["$(Int(p*100)) \\%" for p::Float64 ∈ percentiles];
    "\$\\%_{2016}\$"; "\$>x_{2016}+$(Int(σsAbove2016))\\sigma\$"]
  numCols::Int = length(summaryColNames)

  #make the content matrix
  summaryContent::Vector{Vector{String}} = [Vector{String}(numCols) for i::Int ∈ 1:numRows]


  for r::Int ∈ 1:numRows
    summaryContent[r][1] = string(size(df,1)-sum(isna.(df[:,fields[r]])))
    summaryContent[r][2] = formNum(mean(dropna(df[:,fields[r]])),decimals)
    summaryContent[r][3] = formNum(std(dropna(df[:,fields[r]])),decimals)

    #get the percentiles
    for c ∈ 1:length(percentiles)
      summaryContent[r][c+nonPercentileNumCols] =
        formNum(quantile(dropna(df[:,fields[r]]), percentiles[c]),decimals)
    end
    summaryContent[r][end-1] = isna(df[end,fields[r]]) ? "" :
        formPercentile(df[end,fields[r]], ecdf(dropna(df[:,fields[r]])), decimals)

    numGreater::T where T <: Union{Int, NAtype} = sum(dropna(df[:,fields[r]]) .≥
    df[df[:,:year].==2016,fields[r]] + std(dropna(df[:,fields[r]])) * σsAbove2016)

    summaryContent[r][end] = isna(numGreater) ? "" : (
  ...
  TableSummarySD::String = texTable( "Summary of ST and LT Interest Yields",
    ...
    ltecdf::Function = ecdf(dropna(df[1:(end-1),:tnote20at]))
    ltrate::Float64 = (1+df[end,:tnote20] * (1-df[end,:munitax]))/
      (1+df[end,:tipsinflation]) - 1
    stecdf::Function = ecdf(dropna(df[1:(end-1),:tnote1at]))
    strate::Float64 = (1+df[end,:tnote1] * (1-df[end,:munitax]))/
      (1+df[end,:tipsinflation]) - 1
    println("Quantile of breakeven 20yr: ", ltecdf(ltrate))
    println("Quantile of breakeven 1yr: ", stecdf(strate))

    writeTables2File([TableSummarySD],
      HEADER_NAME, FOOTER_NAME, path=OUTPUT_PATH,
      outName = "TableSummary.tex")
...
function makeReturnGraphs!(taxDFm::DataFrame, taxDFq::DataFrame, taxDFy::DataFrame;
  ...
  for iInd ∈ 1:length(inflationFields)
    for rInd ∈ 1:length(returnFields)
      ...
      for tInd ∈ 1:length(taxFields)
        ...
        #record the data
        outDF[:,newField] = ones(size(outDF,1))
        outDF[:,newField] .= NA
      ...
      push!(plots, plot(
          ...
          Guide.manual_color_key("Legend",
            ...
            palette[1:3])))
      push!(plots, plot(
        layer(plotDF[completecases(plotDF[:,[:year,statNames[4]]]),:], x="year", y=statNames[4], Geom.line,Theme(line_width=1.5pt, default_color=palette[1])),
        layer(plotDF[completecases(plotDF[:,[:year,statNames[5]]]),:], x="year", y=statNames[5], Geom.line,Theme(line_width=1.5pt, default_color=palette[2])),
        layer(plotDF[completecases(plotDF[:,[:year,statNames[6]]]),:], x="year", y=statNames[6], Geom.line,Theme(line_width=1.5pt, default_color=palette[4],
          line_style=:dash)),
  ...
  push!(plots, plot(
      ...
      layer(plotDF[completecases(plotDF[:,[:year,summaryRates[4]]]),:], x="year", y=summaryRates[4], Geom.line,Theme(line_width = 1.5pt,
    ...
    push!(plots,plot(
      ...
      layer(plotDF[completecases(plotDF[:,[:year,summaryRates[11]]]),:], x="year", y=summaryRates[11], Geom.line,Theme(line_width = 1.5pt,
        default_color=palette[5],   line_style=:dot)),
      layer(yintercept=[0.0], Geom.hline(color=colorant"black", size=0.75pt)),
      Guide.ylabel("Rate"), Guide.xlabel(nothing),
      Guide.xticks(ticks=collect(1910:20:2020)),
  ...
  plotDF[:, :cumex_ew_tot_tax_h] =
    (plotDF[:,:div_levels] .- minimum(dropna(plotDF[:,:div_levels]))) ./
    (maximum(dropna(plotDF[:,:div_levels])) .- minimum(dropna(plotDF[:,:div_levels]))) .* .1
  ...
  push!(plots, plot(
      ...
      layer(plotDF[completecases(plotDF[:,[:year,summaryTaxRates[4]]]),:], x="year", y=summaryTaxRates[4], Geom.line,Theme(line_width = 1.5pt,
      ...
      Coord.cartesian(xmin=1900, xmax=2020, ymin=0.0, ymax=1.0),
      style(key_position = :right, key_max_columns = 7, key_label_font_size=8pt, key_label_font="courier"),
      Guide.manual_color_key("", ["Top Ordinary ", "Top Dividend (dash)", "Avg Marg (Ord.)",  "LT Gains"],
        palette[[2,4,1,3]])))
  ...
  for i ∈ 1:length(plots)
    draw(SVG("$OUTPUT_PATH\\$(plotNames[i]).svg", 8inch, 2inch),plots[i])
    writetable("$OUTPUT_PATH\\$(plotNames[i]).csv", graphFrames[i])
  end
...
function getFieldNames(t::TaxSpec)::Vector{Symbol}
  return [t.taxField, t.muniField, t.muniTField, t.corpField, t.corpTField,
    t.muniSpread, t.corpSpread, t.TAT]
end
...
function TaxSpec(; indName::String = "", depName::String = "",
  ...
  return TaxSpec(indName, depName, taxField, muniField, muniTField, corpField,
    corpTField, muniSpread, corpSpread, TAT)
end
...
function prepRegression(df::DataFrame, taxSpecs::Vector{TaxSpec}, accountForStates::Bool = false)::DataFrame
  ...
  for t::TaxSpec ∈ taxSpecs
    ...
    #println(dropna(df[:,t.corpSpread]))
  end
...
function standardReg(df::DataFrame, taxSpecs::Vector{TaxSpec},
  ...
  rowNames::Vector{String} =
    [ "20Y T-Bond Minus AAA Corp" ;["20Y T-Bond*"*t.indName for t::TaxSpec ∈ taxSpecs]; otherSeriesNames; "Intercept"]

  for t::TaxSpec ∈ taxSpecs
    push!(XSpecs, parse("$(t.TAT) + $(t.corpSpread)"))
    push!(XNames, [:intercept; t.taxField; :corpSpread])
    push!(YSpecs, t.muniSpread)
    push!(colNames, t.depName)

    push!(XSpecs, parse("$(t.TAT) + $(t.corpSpread) + $(otherSeriesStr)"))
    push!(XNames, [:intercept; t.taxField; :corpSpread; otherSeriesSyms])
    push!(YSpecs, t.muniSpread)
    push!(colNames, t.depName)
  end
  ...
  descContent::Vector{Vector{String}} =
    [Vector{String}(length(models))for i∈ 1:length(descRowNames)]

  for i ∈ 1:length(XSpecs)
    descContent[1][i] = "$(models[i].N)"
    descContent[2][i] = "$(round(getR(models[i])^2.0,2))"
  end

  #println(getFieldNames(taxSpecs[1]))
  taxSyms::Vector{Symbol} = [t.taxField for t::TaxSpec ∈ taxSpecs]

  TableText::String = texTable(models, getModWhiteΣ!, [:corpSpread; taxSyms; otherSeriesSyms; :intercept],
...
function difference!(vec::DataVector{Float64}, putForward::Bool = true)::Void
...
function getSDTable(df::DataFrame, fields::Vector{Symbol}, fieldNames::Vector{String};
  ...
  σsDA::Vector{Float64} = ((s::Symbol)->std(dropna(dfDA[:,s]))).(fields)
  μsDA::Vector{Float64} = ((s::Symbol)->mean(dropna(dfDA[:,s]))).(fields)
  cdfs::Vector{Function} = ((s::Symbol)->ecdf(dropna(dfDA[:,s]))).(fields)
  ...
  colNames::Vector{Vector{String}} =
    ...
    ["\$(\\mu=$(round(μs[i],2)), \\sigma=$(round(σs[i],2)))\$" for i::Int ∈ 1:K],
  ...
  #get the content
  rows::Vector{Vector{String}} = [Vector{String}(3*K) for r::Int ∈ 1:N]
  for r::Int ∈ 1:N
    for c::Int ∈ 1:K
      val::T where T<:Union{NAtype,Float64} = df[r,fields[c]]
      valDA::T where T<:Union{NAtype,Float64} = dfDA[r,fields[c]]
      rows[r][c*3-2] = isna(val)?"":"$(round(val*100.0,2))"
      rows[r][c*3-1] = isna(valDA)?"":"$(round((valDA*100.0 - μsDA[c])/σsDA[c],2))"
      rows[r][c*3] = isna(valDA)?"":"$(round(cdfs[c](valDA)*100.0,2))"
    end
  end
  ...
  if writeSDTable
    ...
    writetable("$OUTPUT_PATH\\resultsSD_$label.csv",df[:,[:year; colSyms]], nastring="")
  end
...
function getRegressionTables!(taxDFm::DataFrame, taxDFq::DataFrame, taxDFy::DataFrame)::Void
  ...
  for s ∈ [:top_div_rate, :us_wage_marg_rate_1000, :us_interest_marg_rate,
    ...
    taxDFm[:,s] = flattenYears(taxDFy[:,:year], taxDFy[:,s], taxDFm[:,:date])
    taxDFq[:,s] = flattenYears(taxDFy[:,:year], taxDFy[:,s], taxDFq[:,:date])
  end
...
function meanHoldings!(taxDFq::DataFrame, taxDFy::DataFrame;
  ...
  for f::Symbol ∈ fieldList
    taxDFy[:,f] = DataVector{Float64}(Vector{Float64}(size(taxDFy,1)))
    taxDFy[:,f] = NA
  end
  ...
  by(taxDFq, :year) do taxDFqSub::SubDataFrame
    ...
    for f::Symbol ∈ fieldList
      if sum((!).((isna).(taxDFqSub[:,f]))) > 0
        taxDFy[yearInd[yr], f] = mean(dropna(taxDFqSub[:,f]))
      end
    end
  end
...
function processTaxData(;refreshWorkingData = true, replaceYahooData=true)::Void
  ...
  sort!(taxDFm, cols = [:date])
  sort!(taxDFq, cols = [:date])
  sort!(taxDFy, cols = [:year])
  ...
  writetable("$DATA_PATH\\$(DATA_FILE_NAME)-y.csv",taxDFy, nastring="")
  writetable("$DATA_PATH\\$(DATA_FILE_NAME)-q.csv",taxDFq, nastring="")
  writetable("$DATA_PATH\\$(DATA_FILE_NAME)-m.csv",taxDFm, nastring="")
...
@time begin
  processTaxData(refreshWorkingData = true, replaceYahooData =  false)
end
