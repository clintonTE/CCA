...
using DataFrames, Distributions, CSV, StatsBase, GZip, ForwardDiff,# Query,
  Gadfly, NLopt, StaticArrays, Measures, Formatting
...
function graphTrends(longDF::DataFrame)::Nothing
  ...
  push!(plots, plot(aggDF, x=:oldDate, y=:value_mean, color=:exchange,
      Stat.xticks(majorDates), Geom.line, Guide.xlabel("Year"),
...
function processLong!(wideDF::DataFrame)::DataFrame
  ...
  delete!(longDF, :variable)
  longDF =
    longDF[(d::Date->d ∈ minimum(FOCAL_DATES):maximum(FOCAL_DATES)).(longDF[:,:date]),:] #filter to the correct dates
  ...
  longDF[:, :DD1] = (d::Date->MInt(
      d ∈ minimum(FOCAL_DATES_DD1):maximum(FOCAL_DATES_DD1))).(longDF[:, :date])
  longDF[:, :DD2] = (d::Date->MInt(
      d ∈ minimum(FOCAL_DATES_DD2):maximum(FOCAL_DATES_DD2))).(longDF[:, :date])
