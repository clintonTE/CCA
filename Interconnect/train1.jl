...
function summaryVTable(wideDFV::DataFrame,
  ...
  columnNames::Vector{Vector{String}} = [["Shanghai Volume \$\\times 10^{-12}\$", "Shenzhen Volume \$\\times 10^{-12}\$"], ["Shr Vol", "N.B. Buys",
      "N.B. Sells", "Total N.B. Vol", "Shr Vol", "N.B. Buys", "N.B. Sells", "Total N.B."]]
...
function graphTrends(longDF::DataFrame)::Void
  ...
  D10::Date = Date("2010-12-31", dateFormat)
  D11::Date = Date("2011-12-31", dateFormat)
  D12::Date = Date("2012-12-31", dateFormat)
  D13::Date = Date("2013-12-31", dateFormat)
  D14::Date = Date("2014-12-31", dateFormat)
  D15::Date = Date("2015-12-31", dateFormat)
  D16::Date = Date("2016-12-31", dateFormat)
  D17::Date = Date("2017-12-31", dateFormat)
  #majorDates::Vector{Date}= [D10, D11, D12, D13, D14, D15, D16, D17]

  push!(plotNames, "Parallel Trends")
  push!(plots, plot(aggDF, x=:oldDate, y=:value_mean, color=:exchange,
      Geom.line, Guide.xlabel("Year"),Guide.ylabel("Average Firm Cashflow Accrual"),
      Coord.Cartesian(xmin=D10, xmax=D17), Guide.colorkey(title="Exchange:"),
      style(key_position=:bottom, guide_title_position=:left),
      xintercept=[D14, D16], Geom.vline(style=[:solid, [1mm,1mm]])))
...
function pruneLong!(longDF::DataFrame)::DataFrame
  ...
  longDF[(longDF[:, :index] .== :SZSECOM) & ((f::MFloat64-> begin
    if ismissing(f)
      return false
    elseif f < MIN_SZSECOM_MARKETCAP
      return true
    else
      return false
    end
  end).(longDF[:, :marketCap])), :keep] = false
...
function discrete(longDF::DataFrame ;
  ...
  push!(clusteredSpec, :exchange)

  push!(XSpecs, parse("$focalSpec + lMarketCap + date"))
  ...
  push!(XSpecs, parse("$focalSpec + lMarketCap + exchange + date"))
  push!(XNames, [:intercept; focalName; :lMarketCap ])
  push!(YSpecs, YSym)
  push!(withinSpec, :gics)
  push!(clusteredSpec, :gics)
  ...
  push!(XSpecs, parse("$focalSpec + lMarketCap + exchange"))
  push!(XNames, [:intercept; focalName; :lMarketCap])
  push!(YSpecs, YSym)
  push!(withinSpec, :gicsXDate)
  push!(clusteredSpec, :gicsXDate)
