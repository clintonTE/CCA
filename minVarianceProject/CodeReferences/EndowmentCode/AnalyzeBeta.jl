...
function βdf(data::NCCSData; minPointsForβ::Int = MIN_POINTS_FOR_BETA)
  ...
  p = plot(βdf[(f->abs(f)<1.1).(βdf[:b1contributions]),:], x=:b1contributions,
  ...
  p = plot(βdf[(f->abs(f)<1.1).(βdf[:b1contribonret]),:], x=:b1contribonret,
    ...
    p = plot(βdf[(f->abs(f)<1.1).(βdf[:b1expenses]),:], x=:b1expenses,
      Guide.title("histogram of return on contribution betas"), Geom.histogram(bincount=25))
