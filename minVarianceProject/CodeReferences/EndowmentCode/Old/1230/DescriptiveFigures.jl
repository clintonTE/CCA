...
const WRDS_FLDS = [:indfmt, :curcd, :curncd, :costat, :loc, :mkvalt, :prcc_f,
  :stalt, :stko, :idbflag, :revt, :adrr, :seq, :naics]
const WRDS_OK_MISSING_FLDS = [:adrr, :stalt]
#make the industry distribution plots
function industryCrossDF(data::NCCSData; assetYear::Int = maximum(ASSET_YEARS),
    wrdsPath::String = WRDS_PATH, compustatName::String = COMPUSTAT_NAME)::DataFrame
  ...
  aggdf::DataFrame = by(
    presubdf[:,[:naics2, :naics2name,:totrev, :totexp, :netassets]],
      :naics2name,
      [:naics2name,:totrev,:totexp,:netassets] =>
        x -> (n = length(x.naics2name),
        grossincome=sum(skipmissing(x.totrev .- x.totexp)),
        netwealth=sum(skipmissing(x.netassets))))

  compdf::DataFrame = CSV.read("$wrdsPath\\$compustatName.csv")
  compdf = compdf[WRDS_FLDS]
  compdf = compdf[completecases(compdf[setdiff(WRDS_FLDS, WRDS_OK_MISSING_FLDS)])]

  filter!(df) do r::DataFrameRow
    keep::Bool = (    
      r[:indfmt] == "INDL" &&
      ...
      ismissing(r[:mkvalt] > 0.1 #has a market value > 100k
      #has a closing price
      ...
      r[:revt] > 0.#revenue is reported and > 0
...
function oneOffTasks(data::NCCSData)
  df::DataFrame = data.df[(i::Int->i ∈ 2000:2015).(data.df[:fisyr]),[
    :name, :ein, :fisyr, :charitytype, WEALTH_FOCAL_FIELD, RETURN_FOCAL_FIELD]]
  df = df[df[:charitytype].== :pc, :]
  #println("$(df[1:3,:name])")
  #println("typeof: $((s::Union{Symbol,Missing}->((!ismissing(s)) && occursin("DOWNTOWN",uppercase(s)))).(df[:name]))")
  df = df[(s::Union{String,Missing}->((!ismissing(s)) && occursin("DOWNTOWN",uppercase(s)))).(df[:name]),:]

  df |> CSV.write("$WORKING_PATH\\keydata.csv")
