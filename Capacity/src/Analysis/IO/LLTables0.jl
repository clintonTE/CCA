...
function regmbb(X::Vector{TXcol},y; bssamples, bswidth) where TXcol<:AbstractVector
...
function nullar1(X::AbstractMatrix,y; β, nullar1simulations, interegrated=false
    )

  #draw the coefficients for the bootstrap
  nullar1statfunc(X,r) = Dict(:beta => cholesky!(Symmetric(X'*X))\(X'*r),)


  function nullar1aggfunc(X)
    aggfunc = (;se=(v->std(skipmissing(v))).(eachrow(bsmat)), p=nonparametricp.(eachrow(bsmat)))
  end
  ...
  :measure=>Dict(
    ...
    :L1DEON_WXmomentum121s=>(;
      ...
      se=(;
        ...
        label="",
        format=llseformat)),
    :dtL1M_WXmomentum121s=>(;
      id=:dtL1M_WXmomentum121s,
      label="\$dtM^{L+S}_{t-1}\$",
      format=llbetaformat,
      se=(;
        id=:dtL1M_WXmomentum121s_se,
        label="",
        format=llseformat)),
    :dtL1MT_WXmomentum121s=>(;
      id=:dtL1MT_WXmomentum121s,
      label="\$dtM^{tot}_{t-1}\$",
      format=llbetaformat,
      se=(;
        id=:dtL1MT_WXmomentum121s_se,
        label="",
        format=llseformat)),
    :dtL1AN_WXmomentum121s=>(;
      id=:dtL1AN_WXmomentum121s,
      label="\$dtMF^{all}_{t-1}\$",
      format=llbetaformat,
      se=(;
        id=:dtL1AN_WXmomentum121s_se,
        label="",
        format=llseformat)),
    :dtL1DEON_WXmomentum121s=>(;
      id=:dtL1DEON_WXmomentum121s,
      label="\$dtMF^{dom. eq.}_{t-1}\$",
      format=llbetaformat,
      se=(;
        id=:dtL1DEON_WXmomentum121s_se,
        label="",
        format=llseformat)),
      :L12M_WXmomentum121s=>(;
      ...
      :L12DEON_WXmomentum121s=>(;
        ...
        se=(;
          ...
          label="",
          format=llseformat)),
      :dtL12M_WXmomentum121s=>(;
        id=:dtL12M_WXmomentum121s,
        label="\$dtM^{L+S}_{t-12}\$",
        format=llbetaformat,
        se=(;
          id=:dtL12M_WXmomentum121s_se,
          label="",
          format=llseformat)),
      :dtL12MT_WXmomentum121s=>(;
        id=:dtL12MT_WXmomentum121s,
        label="\$dtM^{tot}_{t-12}\$",
        format=llbetaformat,
        se=(;
          id=:dtL12MT_WXmomentum121s_se,
          label="",
          format=llseformat)),
      :dtL2AN_WXmomentum121s=>(;
        id=:dtL12AN_WXmomentum121s,
        label="\$dtMF^{all}_{t-12}\$",
        format=llbetaformat,
        se=(;
          id=:dtL12AN_WXmomentum121s_se,
          label="",
          format=llseformat)),
      :dtL12DEON_WXmomentum121s=>(;
        id=:dtL12DEON_WXmomentum121s,
        label="\$dtMF^{dom. eq.}_{t-12}\$",
        format=llbetaformat,
        se=(;
          id=:dtL12DEON_WXmomentum121s_se,
          label="",
          format=llseformat)),
    :L1F_ff5_mktrf=>(;
      id=:L1F_ff5_mktrf,
...
tablelt136spec = (;
  ...
  Freturns=[[:FW36R_WXmomentum121s for i ∈ 1:3]...;[:dtXFW36R_WXmomentum121s for i ∈ 1:3]...;],
  FN=:Nmonths,
  Fse=:se,
...
function lltables(;analysispath = PARAM[:analysispath],)
  ...
  bssamples = PARAM[:tabllbssamples]
  ...
  lltable(measure00, (X,y)->regmbb(X,y, bswidth=ll00bswidth; bssamples).beta.se;
    table00spec...)

  measurelt136 = CSV.File(
    "$analysispath\\$(PARAM[:tablllt136measurefilename])" *
    "\\$(PARAM[:tablllt136fundrid])\\$(PARAM[:tablllt136fundrid])_leadlag_series.csv"
    ) |> DataFrame
  lt136bswidth = nrow(measurelt136)^(1/3) |> ceil |> Int
  lltable(measurelt136, (X,y)->regmbb(X,y, bswidth=lt136bswidth; bssamples).beta.se;
    tablelt136spec...)
  ...
  lltable(measurest11, (X,y)->regmbb(X,y, bswidth=st11bswidth; bssamples).beta.se;
    tablest11spec...)
  ...
  lltable(measurest21, (X,y)->regmbb(X,y, bswidth=st21bswidth; bssamples).beta.se;
    tablest21spec...)
  ...
  lltable(measurestp, (X,y)->regmbb(X,y, bswidth=stpbswidth; bssamples).beta.se;
    tablestpspec...)
