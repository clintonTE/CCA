...
function mbbootstrap(dat::Vector{Tv}, statfunc::Tstatfunc, aggfunc::Taggfunc,
    ::Type{Tstat}=MFloat64;
    B::Int, w::Int) where {Tv, Tstatfunc, Taggfunc, Tstat}

  #pre-allocate vectors used for the bootstrap
  sims = [deepcopy(dat) for t ∈ 1:Threads.nthreads()]
  results = Dict(s => Vector{Tstat}(undef, B) for s ∈ keys(statfunc(dat)))
  ...
  #=Threads.@threads =#for b ∈ 1:B
    ...
    for s ∈ keys(res)
      @assert (!isassigned(results[s], b)) || (results[s][b] ≡ missing)
      results[s][b] = res[s]
    end
  end
