...
llikelihoodTobitGrad = function(bs, Y, X) {
    epsilon = CONST$EPSILON
    k = length(bs)
    b = bs[1:(k - 1)]
    s = max(abs(bs[k]), epsilon)
    ...
    premults = ifelse(Y > 0, yxbs / s, - (dnorms / pnorms / s))
    #premults = ifelse(is.finite(premults), premults, -10^10)

    #compute the gradients for b and s
    gradsb = apply(X, MARGIN = 2, function(x) ifelse(is.finite(x * premults),x*premults, -10^10))
    gradss = ifelse(Y > 0, yxbs * yxbs / s - 1 / s, (dnorms / pnorms * xbs / s))
    gradss = ifelse(is.finite(gradss), gradss, -10 ^ 10)


    return(c(colSums(gradsb),sum(gradss)))
}
...
tobitModel = function(Y, X, yname=NULL, xnames=NULL, suppressintercept = FALSE) {
    ...
    llgrad = function(x) - 1.0 * llikelihoodTobitGrad(x, Y, X)

    #call the optimizer
    opt = optim(bs, fn=ll, #gr = llgrad,
...
runTobitModelOnce = function() {
    ...
    predictTobit = function(xb, s) pnorm(xb/s) + dnorm(xb/s) * s
