...
function mergeewindd!(crsp::DataFrame)
    vwcrsp = loadidxcrsp(VWCRSP_NAME, RETAINED_VWCRSP_COLS)

  ewidx::Dict{Date, Float64} = interpolateindex(vwcrsp[!, :ewindd], vwcrsp[!, :date])
  ...
  crsp.ewindd = Vector{MFloat64}(undef, size(crsp,1))

  @assert length(crsp.date) == length(crsp.ewindd)
  @mpar for i ∈ 1:length(crsp.date)
    crsp.ewindd[i] = ewidx[crsp.date[i]]
  end
  #crsp.ewindd .= (d::Date->ewidx[d]).(crsp.date)
  crsp.lewindd = (log).(crsp.ewindd)
