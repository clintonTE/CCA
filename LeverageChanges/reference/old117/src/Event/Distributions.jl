...
function readdist(;fname::String = CRSP_DISTRIBUTIONS_NAME,
  ...
  dist = incsvstream("$path\\$fname.csv.lz4") |> CSV.read |> DataFrame
...
function altreaddist(;  prefix = error("prefix is required"),
  path::String=EVENT_PATH,
  incsvstream::Function = IN_CSV_STREAM,
  crspdateformat::DateFormat = CRSP_DATE_FORMAT,
  refreshaltdist::Bool=true,
  )

  local dist::DataFrame = prepcrsp(
    crspcolumns = [:date, :permno, :divamt, :distcd, :dclrdt],
    crsptype = REPLICATION_TYPE[],
    refreshcrsp=refreshaltdist,
    validpermnos = nothing,
    prefix = "altdist") do df::DataFrame
