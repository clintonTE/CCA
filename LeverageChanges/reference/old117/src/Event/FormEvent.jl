...
function formevent(;refreshevent::Bool = true,
  ...
  if   refreshevent
    ...
    if DIST_TYPE[] ∈ [:short, :full]
      dist = readdist()
    elseif DIST_TYPE[] ∈ [:altshort, :altfull]
      dist = altreaddist(prefix="", refreshaltdist=refreshdistcrsp)
    else
    ...
    (DIST_TYPE[] == :short || DIST_TYPE[]==:altshort) && (
      dist = dist[rand(1:(size(dist,1)),5000),:])  #WARNING TEST DEBUG ONLY
