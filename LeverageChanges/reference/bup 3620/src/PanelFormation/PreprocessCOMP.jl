...
function prepcomp(;companame=COMP_A_NAME, compqname = COMP_Q_NAME, compname = COMP_NAME,
  ...
  local compq::Union{DataFrame,Nothing} = nothing #keep this optional to better match annual data results
  local comp::DataFrame

  #makes a serialization object so we don't have to keep parsing the CSV
  if refreshcomp
    compa = incsvstream("$datapath\\$companame.$csvextension") |> CSV.read |> DataFrame
    usequarterlydates && (compq = incsvstream("$datapath\\$compqname.$csvextension") |> CSV.read |> DataFrame)
...
function preprocesscomp(compa::DataFrame, compq::Union{DataFrame,Nothing};
...
function preprocesscomp!(comp::DataFrame)
  ...
  rename!(comp, (s::Symbol->Symbol(lowercase(string(s)))).(names(comp)))
...
function preprocesscompa!(compa::DataFrame;
  ...
  @mpar for s ∈ [:netdebt, :bkequity, :at, :lt] #can thread this a bit
  ...
  compa.op = (boundabsunit).(compa.pi ./ compa.bkequity)
  ...
  issorted(compa, [:gvkey, :fyear]) || error("compa not sorted on fyear")
  lagwithin!(compa, [:at, :fyrmonth], :gvkey, :fyear, sorted=true)
...
function preprocesscompq!(compq::DataFrame;
  ...
  rename!(compq, (propername).(names(compq)))
  ...
  filter!(r::DataFrameRow->year(r.fdateq) ∈ yearrange, compq) #restrict to desired years
...
function dedupcomp!(comp::DataFrame,
  ...
  IVO_REP && (comp = comp[comp.indfmt .== :INDL,:])
  ...
  @mpar for i ∈ 1:length(scomps)
    ...
    if size(scomp,1) > 1
      ...
      if sum(scomp.keep) > 1
        IVO_REP && error("duplicate rows after dropping fs records")
