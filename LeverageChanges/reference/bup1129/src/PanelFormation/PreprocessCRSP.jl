...
@inline normalizecrspnames!(crsp::DataFrame) = names!(crsp, (s::Symbol->Symbol(lowercase(string(s)))).(names(crsp)))
...
function prepcrsp(op!::Function = (noop(x)=x);
  ...
  if refreshcrsp
    ...
    crsp = CSV.read(incsvstream("$pin.csv.lz4"), threaded=CSV_THREADED) |> DataFrame
    ...
    if length(crspparts) > 1
      ...
      dftypes::Vector{Type} = eltypes(crsp)
      for crsppart ∈ crspparts[2:end]
        pin = "$datapath\\$crsppart"
        local addedcrsp::DataFrame = CSV.read(
          incsvstream("$pin.csv.lz4"), types=dftypes,threaded=CSV_THREADED) |> DataFrame
    ...
    #seems to sometimes parse this as a string
    if (:numtrd ∈ names(crsp)) && (String <: eltype(crsp.numtrd))
      crsp.numtrd = (i::MString->parsecrsp(Int, i)).(crsp.numtrd)
    end
    ...
    local istream  = outjlsstream("$p.jls.lz4")
    local wrotefile::Bool = false
    #=try
      serialize(istream, crsp)
      wrotefile = true
    finally
      flush(istream)
      close(istream)
      wrotefile || error("Write to file failed $err")
    end=#
