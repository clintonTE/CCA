...
function table1panelB(panel::DataFrame;
  ...
  desccontent::Vector{Vector{String}} = (
    i->Vector{String}(undef, length(CQUART_FVALS))).(1:length(T1_PANEL_B_ROWKEYS))
  ...
  for c ∈ 1:length(CQUART_FVALS)
    ...
    desccontent[T1_PANEL_B_IDX[:leverageQmeans]][c] = (
      "$(n2s(q1val, 1)) \\; $(n2s(q4val, 1))")
    desccontent[T1_PANEL_B_IDX[:quartilenum]][c] = ("\\text{Q1 Q4}")
    ...
    desccontent[T1_PANEL_B_IDX[:LLsd]][c] = (
      "$(n2s(q1LLvol)) \\; $(n2s(q4LLvol))")
    ...
    desccontent[T1_PANEL_B_IDX[:sd]][c] = (
      "$(n2s(q1vol)) \\; $(n2s(q4vol))")
    ...
    desccontent[T1_PANEL_B_IDX[:Δsd]][c] = (
      "$(n2s(Δq1vol)) \\; $(n2s(Δq4vol))")
    ...
    desccontent[T1_PANEL_B_IDX[:Δret12m]][c] = (
      "$(n2s(Δq1ret)) \\; $(n2s(Δq4ret))")
  end
