...
function FMNeweyWest(fs::FactorSpecification,
  ...
  X::Matrix{Float64} = [ones(size(fs.df,1)) Matrix{Float64}(fs.df[!, coefnames[2:end]])]
  K::Int = length(λᵢ)
  ...
  for sdf ∈ fs.eachport
    ...
    Sₜ::Matrix{Float64} += BLAS.gemm('T','N',1.0/T, Xₑ, Xₑ)

    for v::Int ∈ 1:lags
      #overwrites temp with (1/N)R'R
      BLAS.gemm!('T', 'N', 1.0/T, view(Xₑ, (v+1):T, :),view(Xₑ, 1:(T-v), :), 0.0, temp)
  ...
  BLAS.gemm!('N','N',Float64(size(X,1)), RRinvS, RRinv, 0.0, Σ)
...
function FMNeweyWestSlow(fs::FactorSpecification,
  ...
  return size(X,1) * XXInv * Sₜ * XXInv
end
...
function characteristicFM(fs::FactorSpecification; parallel::Bool = false, lags=2)::FactorRegression
  ...
  println("FastNW: $σᵢ SlowNW: $(diag(FMNeweyWestSlow(fs, coefnames, λᵢ, lags)).^0.5) NormalSE: $((c->std(c)).(eachcol(λ)) ./ t)")
