...
function constructportfolios(;
  ...
  if refreshportfolios
    ...
    if TEST_OUTPUT
      ...
      CSV.write("output\\$(panelname)_ts_$d.csv", panel[(end - 20_000):end, :])
    ...
    retainedpanelcols = unique([:gvkey; :permno; :date; :fyear; :linkeffdate; :linkenddate;
      ...
      :LLvol252d; :annflag; (s->Symbol(:LD, s)).(CQUART_FVALS); CQUART_FVALS_BRS_D;])
    select!(panel, retainedpanelcols)

    sort!(panel, [:permno, :date])
    outbinstream("$datapath\\$panelname.$binextension") do s
      serialize(s, panel)
    end
    #CSV.write("$datapath\\$portname.csv")
  #error("A-OK!")
  else
    inbinstream("$datapath\\$panelname.$binextension") do s
...
const DVOL_FILTER_FIELD = :LDvol252d
