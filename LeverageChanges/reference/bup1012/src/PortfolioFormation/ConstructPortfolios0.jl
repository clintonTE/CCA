...
function constructportfolios(;
  ...
  if refreshportfolios
    ...
    retainedpanelcols = unique([:gvkey; :permno; :date; :fyear; :linkeffdate; :linkenddate;
      ...
      :ret; :lret; T1_CONTROL10; :ret12m; :LLret12m; :vol252d; :Lvol252d;
      :LLvol252d; :annflag; (s->Symbol(:LD, s)).(CQUART_FVALS)])
...
function assigngroupsOLD!(vals::AbstractVector{T},
...
function constructportfolios!(lev::DataFrame)
  ...
  for pc ∈ cqports
    ...
    lev[!, :cgroup] = Vector{MInt}(undef, N) #this is a temporary field holding the 4 within
    #lev[!, pc.Fweight] = Vector{MFloat64}(undef, N)
  end

  #assign the gorups and weights
  #lev.ayear = (year).(lev.adate)
  ...
  for pc ∈ cqports
    ...
    for i ∈ 1:length(sslevs) #for each fiscal year
      ...
      sslev[1:N, :cgroup] .= (i->i÷4).(0:(N-1)) #create the four-group blocks
      s3levs = groupby(sslev, :cgroup)
      @mpar for j ∈ 1:length(s3levs) #for each block
        ...
        s3lev[1:4, pc.Fgroup] .= sortperms[tid]
      end
    end
  end
...
function constructportfoliosOLD!(lev::DataFrame)
  #first construct the portfolios
  local cqports::CQuartileConstructions = leverageconstructions()
