...
function maketable3(panel::DataFrame; trialrunonly::Bool=trialrunonly,
  ...
  T3_PANEL_A_FOCALS::Dict = Dict(
    :mknegcash=>[:Lvol252d, :LLvol252d, :LDmknegcash],
    :bknegcash=>[:Lvol252d, :LLvol252d, :LDbknegcash],
    :mkflev=>[:Lvol252d, :LLvol252d, :LDmkflev],
    :bkflev=>[:Lvol252d, :LLvol252d, :LDbkflev],
    :mkliab=>[:Lvol252d, :LLvol252d, :LDmkliab],
    :bkliab=>[:Lvol252d, :LLvol252d, :LDbkliab])
  ...
  T3_PANEL_A_NAME::String = "t3panela-$(REPLICATION_TYPE[])"
  ...
  runpanela && table3panelA(apanel,
    ...
    T3_PANEL_B_CLUSTERS::Vector{Symbol} = [:fyear, :permno]
    ...
    T3_PANEL_B_NAME::String = "t3panelb-$(REPLICATION_TYPE[])"
...
function table3panelB(panel::DataFrame;
    ...
    colnames::Vector{Vector{String}} = [[
      "\$\\rho_1\$", "\$\\rho_2\$", "\$\\gamma\$",
      "OLS", "CLR-i", "CLR-t", "WHT", "CLR-it"]],
    decimals::Int = DEFAULT_DECIMALS,
    minrowspergroup::Int = error("minrowspergroup is required"),
    errorfunctions::Vector{Function} = "error functions are required",
    fclusters = [:fyear, :permno]
    )
  ...
  for i ∈ 1:length(fss)
    frs::Vector{FactorRegression} = panelFM(fss[i], errorfunctions=errorfunctions)
    push!(vfrs, frs)
  end
