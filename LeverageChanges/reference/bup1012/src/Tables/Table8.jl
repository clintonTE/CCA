...
function maketable8(dist::DataFrame;
  ...
  T8_PANEL_A_NAME::String = "t8panela-$(REPLICATION_TYPE[])"
  ...
  T8_PANEL_A_ROWLABEL::Dict = Dict(
    ...
    :nooverlap => "No Overlaps \$\\pm\$ 12 days",
  ...
  sdist::SubDataFrame = view(dist, dist.nooverlap .* dist.match .* dist.ordinary,:)
  ...
  T8_PANEL_B_NAME::String = "t8panelb-$(REPLICATION_TYPE[])"
  ...
  T8_PANEL_C_NAME::String = "t8panelc-$(REPLICATION_TYPE[])"
...
function table8panela(dist::DataFrame;
  ...
  sdist = view(sdist, sdist.nooverlap, :)
  rowindex[:nooverlap][1] = "\\text{N=}$(length(unique(sdist.eid)))"
