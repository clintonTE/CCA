...
function maketable1(panel::DataFrame; trialrunonly::Bool=trialrunonly,
  ...
  T1_PANEL_A_IDX::Dict = Dict(
    ...
    :std=>2,
    :stdxt=>3)
  T1_PANEL_A_NAME::String = "t1panela-$(REPLICATION_TYPE[])"
  ...
  T1_PANEL_B_ROWLABEL::Dict = Dict(
    ...
    :sd=>"SD Net Lead",
    :Δsd=>"SD Net 2Y Delta",
    :LLret12m=>"\\\\ Compound Raw Lagged\$^2\$",
  ...
  T1_PANEL_B_NAME::String = "t1panelb-$(REPLICATION_TYPE[])"
  ...
  T1_PANEL_C_NAME::String = "t1panelc-$(REPLICATION_TYPE[])"

  T1_PANEL_C_MIN_ROWS::Int = 24
  ...
  T1_PANEL_D_NAME::String = "t1paneld-$(REPLICATION_TYPE[])"
  T1_PANEL_D_MIN_ROWS::Int = 24
...
function tablexpanelA(panel::DataFrame;
  ...
  for c ∈ 1:length(fvals)
    ...
    desccontent[idx[:mean]][c] = num2str(cmean, decimals)
...
function tablexpanelB(panel::DataFrame;
  ...
  #(s->winsorize!(panel[!,s], prop = .999, sorted=false)).([retfields...; volfields...;])
  ...
  @inline meanbyyear(df::AbstractDataFrame, f::Symbol, grp::Symbol = :fyear) = skipmean(
    aggregate(groupby(df[!, [grp, f]], grp), skipmean)[!,Symbol(f, :_skipmean)])

  #function meanbyyear(df::AbstractDataFrame, f::Symbol, grp::Symbol = :fyear)
  #  adf = aggregate(groupby(df[!, [grp, f]], grp), skipmean)
  #  println(names(adf))
#
  #  return skipmean(adf[!,Symbol(f, :_skipmean)])
  #end
  ...
  #make delta fields
  panel.ret4M1 = panel[!, retfields[2]] .- panel[!, retfields[1]]
  for i ∈ 1:length(fvals)
    ...
    desccontent[idx[:leverageQmeans]][c] = (
      "$(num2str(q1val, 2))")
    desccontent[idx[:leverageQmeans]][c+1] = ("$(num2str(q4val, 2))")
    ...
    desccontent[idx[:Δsd]][c] = ("$(n2s(Δq1vol))")
    desccontent[idx[:Δsd]][c+1] = ("$(n2s(Δq4vol))")
    ...
    q1LLret::Float64 = meanbyyear(sspanelq1, retfields[1])#skipmean(sspanelq1[!, retfields[1]])
    q4LLret::Float64 = meanbyyear(sspanelq4, retfields[1])#skipmean(sspanelq4[!, retfields[1]])
    desccontent[idx[:LLret12m]][c] = ("$(n2s(q1LLret))")
    desccontent[idx[:LLret12m]][c+1] = ("$(n2s(q4LLret))")

    q1ret::Float64 = meanbyyear(sspanelq1, retfields[2])
    q4ret::Float64 = meanbyyear(sspanelq4, retfields[2])
    desccontent[idx[:ret12m]][c] = ("$(n2s(q1ret))")
    desccontent[idx[:ret12m]][c+1] = ("$(n2s(q4ret))")

    Δq1ret::Float64 = meanbyyear(sspanelq1, :ret4M1)
    Δq4ret::Float64 = meanbyyear(sspanelq4, :ret4M1)
    desccontent[idx[:Δret12m]][c] = ("$(n2s(Δq1ret))")
    desccontent[idx[:Δret12m]][c+1] = ("$(n2s(Δq4ret))")
...
function tablexpanelBold(panel::DataFrame;
    fvals::Vector{Symbol} = error("PANELB: fvals is required"),
    #fgroups::Vector{Symbol} = error("PANELB: fgroups is required"),
    rowkeys::Vector{Symbol} = error("PANELB: rowkeys is required"),
    rowlabel::Dict = error("PANELB: rowlabel is required"),
