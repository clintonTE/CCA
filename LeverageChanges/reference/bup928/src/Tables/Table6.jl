...
function maketable6(panel::DataFrame; trialrunonly::Bool=trialrunonly,
  ...
  T6_PANEL_A_NAME::String = "t6panela-$(REPLICATION_TYPE[])"
...
function table6panela(panel::DataFrame;
  ...
  minipanel = melt(minipanel, idcols, testfocals, variable_name = :portname, value_name = :ret)
  ...
  for (i,rowkey) ∈ enumerate(rowkeys)
    ...
    for (c,colkey) ∈ enumerate(colkeys)
      ...
      desccontent[r][colidx[colkey]] = num2str(α*100, decimals)
      desccontent[r+1][colidx[colkey]] = num2str(t, decimals)
      desccontent[r+2][colidx[colkey]] = num2str(αR²*100, 0)
    end
  end
