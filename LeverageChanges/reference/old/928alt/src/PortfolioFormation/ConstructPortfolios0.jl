...
function constructportfolios(;
  ...
  if refreshportfolios
    ...
    panel = formfactors!(panel, crsptype=dailyunlesstrial)
    panel = mergeoutside(panel)

    checksum::Float64 = computechecksum(panel)
    @info "Panel constructed with checksum $(checksum)"

    if TEST_OUTPUT
      d = Date(2015,12,31)
      CSV.write("output\\panel$(panelname)_$d.csv", panel[(cd->cd==d).(panel.date), :])
    end
...
function formfactors!(lev::DataFrame; Fret::Symbol=FRET, crsptype::Symbol)::NTuple{2,DataFrame}
  ...
  panel = makepanelmonthly(lev, crsptype=crsptype)
  ...
  join(panel, port, on=:date, kind=:inner)
  println("Size of panel: $(size(panel))")
