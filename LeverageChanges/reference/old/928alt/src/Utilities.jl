...
const RETAINED_COLS_DATA_SERIES = unique([:gvkey; :fyear; :fyrmonth;
  ...
  const T1_CONTROL0 = Vector{Symbol}()
  const T1_CONTROL4 = [:Lbm; :Lmkequity; :LDat; :Lop]
  const T1_CONTROL10 = [CONTROL4; :L12ret12m; :L24ret12m; :fyret12m; :L12vol252d; :L24vol252d; :fyvol252d]

  const T1_PANEL_D_CONTROLS = Dict(
  :quartiles=>CONTROL0,
  :quartiles4ctrl=>CONTROL4,
  :quartiles10ctrl=>CONTROL10,
  :values=>CONTROL0,
  :values4ctrl=>CONTROL4,
  :values10ctrl=>CONTROL10,
  :quartiles_ac=>CONTROL0,
  :quartiles_lret=>CONTROL10)

  const T1_PANEL_D_RET = Dict(
  :quartiles=>:ret,
  :quartiles4ctrl=>:ret,
  :quartiles10ctrl=>:ret,
  :values=>:ret,
  ...
  const T1_PANEL_D_ROWNAME = [:quartiles, :quartiles4ctrl, :quartiles10ctrl, :values,
