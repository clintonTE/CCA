...
function prepcrsp(op!::Function = (noop(x)=x);
  ...
  validpermnos::Vector{Int} = getvalidpermnos(),
  prefix::String = "",
  crspcolumns::Vector{Symbol} = [:permno, :date, :ret, :lret],
...
function computetrailing!(F::Function,
  ...
  months2stale::Int = MONTHS_2_STALE_LONG)::Nothing
  ...
  @mpar for i ∈ 1:length(scrsps)
    ...
    if periods ≤ Nscrsp #if there are enough  entries to compute the trailing returns
      for r ∈ periods:Nscrsp
        r₀::Int = r-periods+1 #this si the first row of the period
        ...
        if staledate ≤ mindate
          scrsp[r, Fnew] = F(scrsp[r₀:r , Ftarget])
        end
      end
    end
  end
...
function computetrailingreturns!(crsp::DataFrame, periods::Int;
  ...
  computetrailing!(sum, crsp, periods, Flret, Flnewret, months2stale=months2stale)
...
function computetrailingvol!(crsp::DataFrame, periods::Int;
  ...
  computetrailing!(std, crsp, periods, Fret, Fvol, months2stale=months2stale)
