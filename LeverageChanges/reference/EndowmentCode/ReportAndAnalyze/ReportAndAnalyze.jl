...
function oneOffCategories(data::NCCSData)
  metadf::DataFrame = deepcopy(data.df[data.df[:fisyr] .== 2013, [
...
function constructPrimaryOutput(; refreshFilter::Bool = true,
    ...
    omitreligious::Bool=true)::NCCSData
  ...
  if refreshFilter
    ...
    data = processAll!(data,
      ...
      omitreligious=omitreligious)

    refreshopen990data && prepregressions!(data)
...
function writedata(df::DataFrame; rows::AbstractRange = 1:size(df,1),
  ...
  path::String = "$workingpath\\$(testname)_$datasuffix.csv"
...
function reportandanalyze(;refreshFilter::Bool = true,
    ...
    refreshtails::Bool=true,
    refreshregressionsv2::Bool=true,
    ...
    #end debug code
    (!omitreligious) && @warn "Religous instituions included. Make sure this is intentional."

    refreshDescriptiveFigures && makeDescriptiveFigures(data)
    refreshDescriptiveTables && makeDescriptiveTables(data, omitreligious=omitreligious)
    ...
    refreshopen990data && (!refreshFilter) && prepregressions!(data)
    refreshsensitivity && olssensitivitytables(data)
    refreshregressionsv2 && makeolscontributiontables(data)
