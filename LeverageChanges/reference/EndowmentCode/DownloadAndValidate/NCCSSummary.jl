...
function summarizeNCCSFile(data::NCCSFile)::DataFrame
  ...
  #=for f ∈ fields
    if f ∉ names(dataDF)[data.meta.index[f]]
      println("Name: Expected=$f Actual= $(names(dataDF)[data.meta.index[f]])")
      println("Type: Expected=$(eltype(data.df[f]))
        Actual= $(data.meta.dictionary[f])")
      error("Dictionary dows not match")
    end
  end=#
  ...
  for i::Int ∈ 1:NFields
    ...
    if (String <: T) || (Symbol <: T)
      col .= (s->ismissing(s) ? missing : "$s").(VecT(dataDF[fields[i]]))
    else
