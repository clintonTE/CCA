...
function readMetaIndex(;indexPath::String = DOCUMENTATION_PATH,
  ...
  df::DataFrame = CSV.read("$indexPath\\$fileName.csv",# strings=:raw,
    categorical=false)
...
function validateMetadata(metadata::Vector{MetaDictionary};
  ...
  keeperDF::DataFrame = CSV.read("$indexPath\\$keeperName.csv",# strings=:raw,
    categorical=false)
...
function getMetadata(;refreshMetadata::Bool = false, metadataPath::String = DOCUMENTATION_PATH,
