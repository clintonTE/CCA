...
function preprocesscrsp(crsp::DataFrame, crsptype::Symbol;
  ...
  @mpar for i ∈ 1:length(scrsps)
    ...
    nonmissingdistcds::Vector{Int} = collect(skipmissing(scrsp.distcd))
    if length(nonmissingdistcds) ≠ 0
      scrsp.keep .= scrsp.distcd .== minimum(nonmissingdistcds)
    else
      scrsp.keep .= false
    end
  end
...
function computetrailing!(F::Function,
  ...
  requiredfields::Vector{Symbol} = [Ftarget; Fnew]
