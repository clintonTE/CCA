...
function loadidxcrsp(idxcrspname::String, retainedidxcrspcols::Vector{Symbol};
  ...
  idxcrsp::DataFrame = CSV.File("$idxcrsppath\\$idxcrspname.csv") |> DataFrame
...
function loadff(ffname::String;
  ...
  #rename the columns
  ffnames::Vector{Symbol} = propertynames(ff)
  ffnames[1] = :date
...
function mergeoutsidedf!(port::DataFrame, source::AbstractDataFrame,
  indexsyms = setdiff!(propertynames(source), [:date]); interpolate::Bool = true)
...
function mergeoutside(port::DataFrame)
  ...
  indicesadded::Vector{Symbol} = setdiff([propertynames(vwcrsp); propertynames(rfcrsp);
    propertynames(ff3m); propertynames(ff5)], [:date])
  ...
  #    filter(s::Symbol->string(s)[1:2] ≠ "S_", propertynames(port))])
  #end
  ...
  port = innerjoin(port, outside, on=:date)
