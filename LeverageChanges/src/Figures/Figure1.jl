...
function aggregatebyday(F::Function, dist::AbstractDataFrame; Fgroup::Symbol = :day,
  selectcols::Vector{Symbol} = propertynames(dist), suffix::String = "")
  ...
  aggdf::DataFrame = combine(sdists, valuecols(sdists) .=> F)
  rename!(aggdf, (n::String->replace(n, "_function"=>suffix)).(names(aggdf)))
