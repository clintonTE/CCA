...
#WARNING: the below greatly improves performance, but creates the possibility of a singlular
#matrix. Set to inf to not use this. In any case, its kind of a kludge.
ROW_NUMBER_ASSUMED_SUFFICIENT = 200
...
@inline function notsmallgroups(df::AbstractDataFrame, Fgroup::Symbol;
  ...
  sdfcounts = groupby(dfcount, :group)
  @mpar for i ∈ 1:length(sdfcounts)
    sdfcount = sdfcounts[i]
...
@inline function notsmallgroups(df::AbstractDataFrame, Fgroup::Symbol, allX::Vector{Symbol};
  minrowspergroup::Int = error("minrowspergroup must be assigned"),
  assumeok::Int = ROW_NUMBER_ASSUMED_SUFFICIENT
  )::Vector{Bool}

  #special case where we aren't as worried about a degenerate rhs
  (length(allX)==0) && (return notsmallgroups(df, Fgroup, minrowspergroup=minrowspergroup))

  groups = unique(df[!,Fgroup])

  #this will record the resulting counts
  dfcount::DataFrame = DataFrame(group = df[!,Fgroup])
  dfcount.count = Vector{MInt}(undef, size(dfcount,1))
  dfcountidx = Dict(sdfcount.group[1]=>sdfcount for sdfcount ∈ groupby(dfcount, :group))

  sdfs = groupby(df, Fgroup)
  @mpar for  i ∈ 1:length(sdfs)
    sdf::SubDataFrame = sdfs[i]
    sdfcount = dfcountidx[sdf[1,Fgroup]]

    local rowcount::Int = size(sdf,1)
    if rowcount < assumeok
      #sdfcount.count .= minimum((col->length(unique(col))).(eachcol(sdf[!,allX])))
      for col ∈ allX #check for degenerate cases
        (minimum(sdf[!,col]) == maximum(sdf[!,col])) && (rowcount=0)
      end
    end

    sdfcount.count .= rowcount
  end
...
function FactorSpecification(df::AbstractDataFrame, Fret::Symbol, Fdate::Symbol, Fport::Symbol;
  ...
  local dfnames::Vector{Symbol} = propertynames(df)
  ...
  allX::Vector{Symbol} = filter(s->!isnothing(s), [Ffactors; Fcharacteristics; Fcontrols])
  sdf = view(sdf, completecases(sdf[!, allfields]), allfields)

  skiptimeseriesdfcheck || (sdf = view(sdf, notsmallgroups(sdf, Fport, allX, minrowspergroup=minrowspergroup), :))
  skipcrosssectiondfcheck || (sdf = view(sdf, notsmallgroups(sdf, Fdate, allX, minrowspergroup=minrowspergroup), :))

  (size(sdf,1)==0) && error("no rows left with reasonable sample size. allX: $allX")
...
function rhsnames(fs::FactorSpecification, focalvars::Union{Vector{Symbol},Nothing};
    intercept::Bool = true)::Vector{String}
...
function manyFF(fs::FactorSpecification,
  ...
  coefnames::Vector{String} = factorrhsnames(fs)
  coefidx = Dict{Symbol, Int}(zip((Symbol).(coefnames), 1:(length(coefnames))))
...
function characteristicFM(fs::FactorSpecification,
  ...
  results = crosssectionalregressions(fs, T, M, V,
    aggfunc=aggfunc, parallel=parallel, qrtype=qrtype)
  coefnames::Vector{String} = characteristicrhsnames(fs)


  coefidx = Dict{Symbol, Int}(zip((Symbol).(coefnames), 1:(length(coefnames))))

  #now build the factor regression object
  #need to be careful we have the rows in the correct order
  λ = vcat((transpose).(results)...) #create rows and make result matrix
  λdf::DataFrame = DataFrame(λ)
  datetype = eltype(fs.df[!, fs.Fdate])
  λdf.date = missings(Union{datetype, Missing}, length(fs.eachdate))
  for (i,sdf) ∈ enumerate(fs.eachdate)
    λdf.date[i] = sdf[1,fs.Fdate]
  end
...
function BJSFFrow(fs::FactorSpecification, df::AbstractDataFrame,
  ...
  for (i, Ysym) ∈ enumerate(Ffactors)
    ...
    coefidx::Dict = Dict{Symbol, Int}(zip((Symbol).(Xnames), 1:(length(Xnames))))
