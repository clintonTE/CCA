...
function table9panelbc(cumex::AbstractDataFrame;
  ...
  Xexpr = :yield
  Xnames= (string).([:intercept, :yield])
  Yname::String = string(Ysym)
  reg = FMLM(scumex,  Xexpr, Ysym, Xnames=Xnames, Yname=Yname, containsmissings=false)
  ...
  Xexpr = :Uyield
  reg = FMLM(scumex,  Xexpr, Ysymunit, Xnames=Xnames, Yname=Yname, containsmissings=false)
  ...
  Xnames= (string).([:intercept, :yieldd1d, :exdayret])
  reg = FMLM(scumex,  Xexpr, Ysym, Xnames=Xnames, Yname=Yname, containsmissings=false)
  se = homoskedasticΣ!(reg)
  semw = modifiedwhiteΣ!(reg)
...
function aggregateoverdays(F::Function, dist::AbstractDataFrame, dayrange::AbstractRange;
  Fgroup::Symbol = :eid, selectcols::Vector{Symbol} = propertynames(dist), suffix::String = "",
  colnames::Vector{Symbol} = propertynames(dist), Fcriteria::Symbol = :day)

  sdist::SubDataFrame = view(dist, (g::Union{MInt, Symbol}->
    (!ismissing(g)) && g ∈ dayrange).(dist[!,Fcriteria]), selectcols)

  sdists::GroupedDataFrame = groupby(sdist, Fgroup)
  agg::DataFrame = combine(sdists, valuecols(sdists) .=> F)
  rename!(agg, colnames)
