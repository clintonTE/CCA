...
function maketable8(dist::DataFrame;
  ...
  sdist::SubDataFrame = view(dist, dist.oneex .* dist.match .* dist.ordinary .* dist.inrange .* dist.primaryyearrange,:)
...
function table8panelc(dist::AbstractDataFrame;
  ...
  #NOTE: can remove the below
  #distaggchk = aggregate(groupby(view(sdist, sdist.day.==0,
  #  [:eid, :yield, :winsorizedyield, :dclrexdays, :dclrextradingdays]), :eid), x->x[1])

  dists2agg = groupby(view(sdist, sdist.day.==0,
    [:eid, :yield, :winsorizedyield, :dclrexdays, :dclrextradingdays]), :eid)
  distagg = combine( dists2agg, dists2agg |> valuecols .=> first)

  #@assert ((c1,c2)->all(c1 .=== c2)).(eachcol(distagg), eachcol(distaggchk)) |> all
  #@info "table 8 check passed- can delete the chk code"

  yields::Vector{Float64} = collect(skipmissing(distagg.yield_first))
  ...
  meandays::Float64 = mean(skipmissing(distagg.dclrextradingdays_first))
  ...
  Nwinsorizedyield::Int = sum(skipmissing(distagg.winsorizedyield_first))
...
function table8panela(dist::DataFrame;
  ...
  local sdist::SubDataFrame = view(dist, dist.primaryyearrange, :)
  descrownames::Vector{String} = (s::Symbol->rowlabel[s]).(rowkeys)
  ...
  rowindex[:alldist][1] = "\\text{N=}$(length(unique(sdist.eid)))"

  sdist = view(sdist, sdist.ordinary, :)
