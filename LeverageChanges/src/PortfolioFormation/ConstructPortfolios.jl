...
function constructportfolios(;
  ...
  if refreshportfolios
    ...
    panel = makepanelmonthly(panel, crsptype=dailyunlesstrial, refreshtretcrsp=refreshtretcrsp)
    ...
    panel = innerjoin(panel, port, on=:date)
    (s->finiteormissing!(panel, s)).([:ret; :lret; T1_CONTROL10])
    #winsorizekeydata!(panel, fields2winsorize = [:ret; :lret; :ret12m; :LLret12m; :vol1year; :LLvol1year; T1_CONTROL10])

    checkdfkey(panel, [:date, :lpermno])
    println("Size of panel: $(size(panel))")

    #=open("$ARCHIVE_PATH\\preprocesspaneldesc-$(Dates.format(now(),"yyyymmdd_HHMM"))-oldselect.txt", "w+") do f
      println(f, "size: $(size(panel))\n",
        describe(panel))
    end
    error("stop")=#
    ...
    if TEST_OUTPUT
      ...
      CSV.write("output\\$portname.csv", port[:,
        filter(s::String->s[1:2] ≠ "S_", names(port))])
    end
    ...
    retainedpanelcols = unique([:gvkey; :permno; :date; :fyear; :linkeffdate; :linkenddate;
      :adate; :begindate; :enddate; :bkequity; propertynames(port); Fgroups(cqports); Fvals(cqports);
      :ret; :lret; T1_CONTROL10; :ret1year; :LLret1year; :vol1yearnet; :Lvol1yearnet; :LLvol1yearnet;
...
function constructportfolios!(lev::DataFrame)
  ...
  for pc ∈ cqports
    ...
    sslevs = groupby(slev, :fyear) #WARNING: should this be by fyrmonth or similar?
...
const DVOL_FILTER_FIELD = :LDvol1yearnet
