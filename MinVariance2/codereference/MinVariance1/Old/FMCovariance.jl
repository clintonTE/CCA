...
function Portfolio!(P, wₜ)::Portfolio
  ...
  P.wₜ .= wₜ
  for c ∈ 1:P.U.N #for each column
    for r ∈ 1:P.wvecsize[c] #for each valid value in the column
      P.wvec[c][r] = P.wₜ[P.U.missingmap[c][r]] # get the valud value from the missing map
    end
    adjfactor::Float64 = sum(P.wvec[c])
    P.wvec[c] ./= adjfactor #rescale to account for missing values
  end

  #build the returns (Note: Threads.@threads provides signficant performance gains)
  for c ∈ 1:P.U.N
    P.R[c] = sum(P.wvec[c] .* P.U.returns[P.U.missingmap[c],c])
  end
...
function testFMCov(N=560, M=5000, iter::Int=100)
