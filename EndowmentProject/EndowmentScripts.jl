...
function endowmentTasks()::Nothing
  ...
  #refreshSummary: runs function consolidateSummarfinanciy in NCCSSummary. Refreshes metadata files.
  ...
  #refreshβ= compute regressions of returns on S&P, contributions on S&P, returns on contribtuions etc
  ...
  #also will refresh some prep work for sensitivity regressions. Refreshing the prep
  #work does NOT require the refreshFilter to be run.
  reportandanalyze(refreshFilter = true,
    refreshopen990data = true,
    refreshsensitivity=true,
    refreshregressionsv2=true,
    refreshDescriptiveFigures = true,
    refreshDescriptiveTables = true,
    doOneOffTasks = false,
    refreshβ=false,
    refreshoutsidedata=false,
    refreshpersistence=true,
    refreshtopinstitutions=true,
    refreshtails=true,
    omitreligious=true) #NOTE CAREFUL ABOUT SWITCHING THIS ONE TO TRUE
