...
function mstring2date(datestring::MString, format::T where T<:Dates.DateFormat)::MDate
  local out::MDate

  if ismissing(datestring) || (length(datestring) == 0)
    out = missing
  else
    try
      out = Date(datestring,format)
    catch err
      out = missing
    end
  end
...
function fixfiscal!(data::NCCSData;
  fiscaldateformat = Dates.DateFormat("$FISCAL_DATE_FORMAT"))::Nothing

  #convert fiscal to dates
  rename!(data.df, :datefiscal=>:datefiscalstr)
  data.df[:datefiscal] =
    (s->mstring2date(s, fiscaldateformat)).(data.df[:datefiscalstr])
...
function mergeSP500!(data::NCCSData;
  ...
  data.df[:netret] = Vector{MFloat64}(missing, size(data.df,1))
  data.df[:lnetret] = Vector{MFloat64}(missing, size(data.df,1))
  ...
  data.df[:netret] .= data.df[:return] .- data.df[:t30ret]
  data.df[:lnetret] .= data.df[:lreturn] .- data.df[:lt30ret]
  ...
  #data.df[:pexpenses] = (data.df[:totexp]) ./ data.df[:Lnetassets]

  #trim rediculous values
  for v ∈ [data.df[:netret], data.df[:lnetret]]
