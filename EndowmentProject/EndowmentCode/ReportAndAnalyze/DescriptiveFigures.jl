...
const ASSET_YEARS = [2015, 2010, 2005, 2000, 1998]
const ASSET_YEARS_FULL = collect(1998:2015)
...
function wealthFiguresByYear(data::NCCSData; assetYears::Vector{Int} = ASSET_YEARS,
  ...
  for y::Int ∈ assetYears #main wealth distribution
    ...
    push!(plots, plot(subdf,
      ...
      Guide.xlabel("Wealth"), Guide.ylabel("Count"),  Scale.x_log10,
      Geom.line, Stat.histogram(bincount=50, density=false),
      Coord.cartesian(ymin=0.0, ymax=2500.0, xmin=5.5)
      ))
...
#########!!!!!!!!!!!!!!!######
  push!(plotNames, "Wealth_Density_of_Non-Profits_by_Year")
  ...
  push!(plots, plot(presubdf,
    ...
    Guide.title("Wealth Density of Non-Profits by Year"),
    Guide.xlabel("Wealth"), Guide.ylabel("Density"),
    Scale.x_log10, Geom.line,
    Stat.histogram(bincount=50, density=true),
    Coord.cartesian(ymin=0.0, ymax = 0.5, xmin=5.5)),)
  ...
  push!(plots, plot(presubdf,
    x=focalField, color=:fisyr,
    Guide.title("Wealth Distribution of Non-Profits by Year"),
    Guide.xlabel("Wealth"), Guide.ylabel("Count"),
    Scale.x_log10, Geom.line,
    Stat.histogram(bincount=50, density=false),
    Coord.cartesian(ymin=0.0, ymax = 20_000, xmin=5.5)),)
...
function returnFiguresByCategory(data::NCCSData;
  ...
  push!(plots, plot(presubdf,
    x=focalField, color=:fisyr,
    Guide.title("Return Distribution of Non-Profits by Year"),
    Guide.xlabel("Return"), Guide.ylabel("Amount"),
     Geom.line,
    Stat.histogram(bincount=50, density=false),
