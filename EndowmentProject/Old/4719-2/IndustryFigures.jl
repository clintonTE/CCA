...
function wrdsAggDF(; wrdsPath::String = WRDS_PATH, compustatName::String = COMPUSTAT_NAME,
    outcols::Vector{Symbol} = [:naics2name, :conm, :revt, :at, :seq])::DataFrame
  ...
  filter!(compdf) do r::DataFrameRow
    return (
      r[:indfmt] == "INDL" &&
  ...
  CSV.write("$WORKING_PATH\\largestPublicForProfits.csv", outdf)
...
function industryCrossDF(data::NCCSData; assetYear::Int = INDUSTRY_CROSS_YEAR,
    ...
    outcols::Vector{Symbol} = [:naics2name, :name, :ein, :totrev, :bookassets, :bookliabilities])::DataFrame
  ...
  CSV.write("$WORKING_PATH\\largestNonProfits.csv", outdf)
