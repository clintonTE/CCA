...
function tabbedhistograms(df::AbstractDataFrame;
  ...
  outtablename::String = ("tabhist-$(shortnameindex[rowfield])X$(
    shortnameindex[colfield])BY$(shortnameindex[tabfield])$(suffix)")

  println("got here 3")
  draw(PDF("$outputpath\\$(outtablename).pdf", 9inch, 7inch), finalplot)
...
function writecrosstabtable(tab::CrossTabTable,
  ...
  writeNakedTable(outtable, path=outputpath, outName="$outtabletitle.tex")
