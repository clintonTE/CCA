...
using  DataFrames, Distributions, StatsBase, Dates, Serialization, CSV
...
function preProcessYahoo(path::String, name::String; makeReturns::Bool = true)::Nothing
  dateFormat::String = YAHOO_DATE_FORMAT
  DF::DataFrame = CSV.read("$path\\$name.csv")

  #fix the dates
  rename!(DF, :Date => :DATE)
  rename!(DF, Symbol("Adj Close") => :Adj_Close)


  if makeReturns
    DF[:DATE] = ((s::String)->Date(s, dateFormat)).(
      DF[:DATE])::V where V <: Union{Vector{Union{Date, Missing}},Vector{Date}}
    sort!(DF, [:DATE])
...
function convertToMaxMonth(coarseDate::Date, fineDates::Vector,
  fineData::Vector{Union{T, Missing}}, convertedFineDates::Vector)::T where T<:Real

  maxDate::Date = maximum(fineDates[convertedFineDates.==coarseDate])
  return fineData[something(findfirst(isequal(maxDate),fineDates),0)]
end

dailyToEOM(coarseDates::Vector,
  fineDates::Vector, fineData::Vector{R where R<: Union{T, Missing}}
  ) where T<: Real = fineToCoarse(coarseDates,
    fineDates, fineData, firstdayofmonth.(fineDates), convertToMaxMonth)

dailyToEOQ(coarseDates::Vector,
  fineDates::Vector, fineData::Vector{R where R<: Union{T, Missing}}
  ) where T<: Real= fineToCoarse(coarseDates,
  fineDates, fineData, firstdayofquarter.(fineDates), convertToMaxMonth)

dailyToEOY(years::Vector,
  fineDates::Vector, fineData::Vector{R where R<: Union{T, Missing}}
  ) where T<: Real= fineToCoarse(
...
function fineToCoarse(coarseDates::Vector,
  fineDates::Vector, fineData::Vector{Union{T, Missing}},
  convertedFineDates::Vector,
  conversionFunc::Function)::Vector{Union{T, Missing}} where T<:Real

  #pre-allcoate the output vector
  out::Vector{Union{T, Missing}} = Vector{Union{T, Missing}}(undef, length(coarseDates))
...
function coarseToFine(coarseDates::Vector{Union{Date, Missing}}, coarseData::Vector{Union{T, Missing}},
  ...
  #pre-allcoate the output vector
  out::Vector{Union{T, Missing}} = Vector{Union{T, Missing}}(undef, length(fineDates))
