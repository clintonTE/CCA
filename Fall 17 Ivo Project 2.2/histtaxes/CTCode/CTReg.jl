...
function CTQR(X::Matrix{Float64}; keepX = true)::CTQR
  ...
  QRCompact::LinearAlgebra.QRCompactWY = qr(X)

  Q::Matrix{Float64} = Matrix{Float64}(QRCompact.Q)
  R::Matrix{Float64} = Matrix{Float64}(QRCompact.R)

  #=if size(Q,1) == size(Q,2)
    Q = Q[:,1:size(R,1)]
  else
    @warn "CTQR Q matrix is already thin. Can delete this conditional statement. Loc code 7893."
  end=#
  return CTQR(X,Q,R)
end
...
function CTLM(df::DataFrame,  XExpr::T, YSym::Symbol;
    ...
    if withinSym ≠ nothing
      within::Vector{W where W <: CTData}  = Vector{W where W <: CTData}(undef, dfSub[withinSym])
    end

    #println("flag4.2")
    if clusterSym ≠ nothing
      clusters::Vector{C where C <: CTData}  = Vector{C where C <: CTData}(undef, dfSub[clusterSym])
    else
      clusters = [nothing]
    end
    ...
    if withinSym != nothing
      #println("flag3a")
        ...
        return CTLM(XModelMatrix, Vector{Float64}(undef, dfSub[YSym]),
...
function getR²(lin::CTLM; adjusted::Bool = false, percentage::Bool = false)::Float64
