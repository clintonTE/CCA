        ...
        for r ∈ 1:length(rows)
            if haskey(XNameTbl, rows[r]) #need to check if it exists
                ...
                content[1][r,c] =
                  ...
                  "\$$(round(scaling[r]*modelsβ[c][ind],digits=decimalDigits))\$$starString"
                content[2][r,c] =
                  #"($(round(scaling[r]*modelsσ[c][ind],decimalDigits)))"
                  "(\$$(round(scaling[r]*modelsσ[c][ind],digits=decimalDigits))\$)"
            end
        end
    end
...
function writeTables2File(tables::Vector{String}, headerName::String,
  ...
  header::String = read(iStream, String)
  close(iStream)

  iStream = open("$path\\$footerName")
  footer::String = read(iStream, String)
  close(iStream)
