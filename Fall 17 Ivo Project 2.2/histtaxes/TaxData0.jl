...
using DataFrames, Distributions, StatsBase, GZip, Gadfly
using  CTRegCopy, CTIOCopy, DataSeries, Formatting, CSV
...
function writeSummaryTable(df::DataFrame, fields::Vector{Symbol}, fieldNames::Vector{String};
  ...
  for r::Int ∈ 1:numRows
    ...
    summaryContent[r][end-1] = ismissing(df[end,fields[r]]) ? "" :
        formPercentile(df[end,fields[r]], ecdf(Vector{Float64}(collect(skipmissing(df[:,fields[r]])))), decimals)

    numGreater::T where T <: Union{Int, Missing} = sum(collect(skipmissing(df[:,fields[r]])) .≥
    df[df[:,:year].==2016,fields[r]] + std(skipmissing(df[:,fields[r]])) * σsAbove2016)
  ...
  TableSummarySD::String = texTable( "Summary of ST and LT Interest Yields",
    ...
    ltecdf::Function = ecdf(Vector{Float64}(collect(skipmissing(df[1:(end-1),:tnote20at]))))
    ltrate::Float64 = (1+df[end,:tnote20] * (1-df[end,:munitax]))/
      (1+df[end,:tipsinflation]) - 1
    stecdf::Function = ecdf(Vector{Float64}(collect(skipmissing(df[1:(end-1),:tnote1at]))))
...
function difference!(vec::Vector{Union{Float64, Missing}}, putForward::Bool = true)::Void
...
function getSDTable(df::DataFrame, fields::Vector{Symbol}, fieldNames::Vector{String};
  ...
  cdfs::Vector{Function} = ((s::Symbol)->ecdf(collect(skipmissing(dfDA[:,s])))).(fields)
  ...
  for r::Int ∈ 1:N
    for c::Int ∈ 1:K
      ...
      rows[r][c*3-2] = ismissing(val)?"":"$(round(val*100.0,2))"
      rows[r][c*3-1] = ismissing(valDA)?"":"$(round((valDA*100.0 - μsDA[c])/σsDA[c],2))"
      rows[r][c*3] = ismissing(valDA)?"":"$(round(cdfs[c](valDA)*100.0,2))"
    end
  end
...
function meanHoldings!(taxDFq::DataFrame, taxDFy::DataFrame;
  ...
  for f::Symbol ∈ fieldList
    taxDFy[:,f] = Vector{Union{Float64, Missing}}(size(taxDFy,1))
    taxDFy[:,f] = missing
  end
