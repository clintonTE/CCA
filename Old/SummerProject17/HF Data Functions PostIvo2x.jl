...
using DataFrames, Distributions, StatsBase, GZip, JLD, Gadfly, JuMP,CTMod
#importall CT
...
function preProcessHFData(;FlowsLaggingProcedure::Char = 'U', newBinary::Bool = false)
  ...
  #println("total outflows: ", sum(dropna(HFData[:,:negFlows])))
  #println("total outflows mo≥1: ", sum(dropna(HFData[HFData[:,:monthsNotice].≥1,:negFlows])))
  #println("total outflows mo>1: ", sum(dropna(HFData[HFData[:,:monthsNotice].>1,:negFlows])))
...
function aggregateOnStrategy(HFData::DataFrame;monthsNotice::T = NA)::DataFrame where T<:Union{NAtype,Int}
        ...
        for s::Symbol ∈ strategies
            if haskey(stratTable,s)
                ...
                #=if !equalWeight
                    ...
                    HFAggSub[sInd,:performance] = mean(dropna(HFSub[:,:performance]))
                end=#
            end
        end
    ...
    showall(HFAgg[HFAgg[:,:fDate].==Date("2015-09-30"),
        [:fund_id, :main_strategy, :fDate,:flows, :assets, :totNegLFlows,:totFlowsOnNotice,:performance, :sigma, :var]])
...
function LM(HFData::DataFrame, titleCaption::String, caption::String;
    flowStr::String = "negLFlows", flowStrDesc::String = "Outflows / AUM x 100")::String
...
function PM(HFData::DataFrame, titleCaption::String, caption::String;
    focalStr= "isNoticePeriod", dependentVar=:performance, cluster=false)::String
...
function joinLongAgg(HFLong::DataFrame, HFLongAgg::DataFrame)::DataFrame
    ...
    @simd for i ∈ 1:size(HFLong,1)
        if haskey(dateSymTable, (HFLong[i,:fDate], HFLong[i,:main_strategy]))
            ...
            warn("Join Failure on row $i. Row Info:\n $(HFLong[i,[:fDate,
...
function getCancelledEstimate(HFDataAgg::DataFrame, β::Float64)
    N::Int = length(unique(HFDataAgg[:,:fDates]))

    sort!(HFDataAgg, cols=[:fDates])

    #allocate space for the result
    fDates::Vector{Date} = Vector{Date}(N)
    fDates .= unique(HFData[:,:fDates])
    totCancelled::Vector{Float64} = Vector{Float64}(size(HFData,1))

    mod = Model(solver=NLoptSolver(algorithm=:LD_LBFGS))
    ...
    CTNorm(z) = sqrt2PiInv*e^(-z^2.0/2.0)
    function obj(μ,σ,t)
        z = (t-μ)/σ
    ...
    #commands to register all the functions and variables
    #The idea is to pre-register so the solver can hot-start
    ...
    for i::Int ∈ 1:Size(HFDataAgg,1)
        Fix(μ, HFSub[i,:performance])
        Fix(σ, HFSub[i,:sigma])

        output = solve(mod)
        HFSub[i,:threshold] = getvalue(t)
        HFSub[i,:totSubmitted] = HFSub[i,:totNegFlows]/
            CTCumNorm((HFSub[i,:threshold]-HFSub[i,:performance])/HFSub[i,:sigma])
        HFSub[i,:canceled] = HFSub[i,:canceled]+HFSub[i,:totNegFlows]
...
function analyzeAggregates(;FlowsLaggingProcedure::Char = 'U',
    ...
    clustered either by fund (4 \\& 8), strategy (3 \\& 7), or date (2 \\& 6)."""
    TableTextPM = runPM?PM(HFData,titleCaption,caption, cluster=true):""
    ...
    titleCaption = "Effect of Notice Period on Performance (No-overlap, 1mo Notice)"
...
@time begin
FLPChar = 'C'
#preProcessHFData(FlowsLaggingProcedure=FLPChar, newBinary=false)
