...
export VAR, VAREstimate, VARPrediction, varΣ, propagateImpulse

importall CTReg, CTIO

const Structure = Union{Void, Vector{Float64}}
const CoefficientErrors = Union{Void, Vector{Float64}}
const RegressionSS = Union{Void, Float64}
const RegressionN = Union{Void, Int}
...
function VAR(ϕ₀::Vector{Float64}, ϕ₁::Matrix{Float64}, L::Matrix{Float64},
  ...
  #calculate the mean
  K::Int = length(ϕ₀)
  μ::Vector{Float64} = ((eye(K) .- ϕ₁)\eye(K))*ϕ₀
...
function VAREstimate(DF::DataFrame, LHS::Vector{Symbol}, RHS::Vector{Symbol};
      ...
      ϕ::Matrix{Float64}=Matrix{Float64}(K,K+1), #these are available for pre-allocaiton purposes
      σ::Vector{Float64} = Vector{Float64}(K))
  ...
  XExpr::CTExpr = parse(vec2String(RHS, " + "))
  ...
  L::Matrix{Float64} = chol(cov(hcat(ε...)))' #(splat and calculate the cholesky root)
