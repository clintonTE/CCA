...
BLAS.set_num_threads(Int(round((Sys.CPU_CORES*3÷4))))
  ...
  const SIG_REDEMPTION_THRESHOLDS = 0.0:0.02:0.1
...
function processLongHFData(;FlowsLaggingProcedure::Char = 'U')
    ...
    HFLong::DataFrame = melt(HFData, [:negLFlows,:months_notice])
...
##################IO Code for 2SLS
    ...
    TableText1stStage::String = texTable(stage12SLS, getHomoskedΣ!, [:intercept; Z2SLSNames[1][1]],
        ...
        contentRowNames = ["intercept", "$(X2SLSSpecs[1]) x100"],
    ...
    thresholdSyms::Vector{Symbol} = getSigRedemptionThresholds()
    modelsRD::Vector{CT2SLS} = Vector{CT2SLS}()

    XRDSpecs::Vector{CTExpr} = Vector{CTExpr}()
    WRDSpecs::Vector{CTExpr} = [Symbol("")]
    ZRDSpecs::Vector{CTExpr} = [parse("isRedemptionDate+0")]

    WRDNames::Vector{Vector{Symbol}} = [[:intercept]]
    XRDNames::Vector{Vector{Symbol}}  = Vector{Vector{Symbol}}()
    ZRDNames::Vector{Vector{Symbol}}  = [[:isRedemptionDate]]

    #push the specifications (use a common name to make a clean table)
    for s::Symbol ∈ thresholdSyms
        push!(XRDSpecs, parse("$s+0"))
        push!(XRDNames, [:negLFlows])
    end

    #run the model
    for iX ∈ 1:length(XRDSpecs), iW ∈ 1:length(WRDSpecs), iZ ∈ 1:length(ZRDSpecs)
        push!(modelsRD, CT2SLS(HFLong, XRDSpecs[iX], WRDSpecs[iW],
            :value, ZRDSpecs[iZ], XNames=XRDNames[iX],
            WNames = WRDNames[iW], YName = :performance, ZNames = ZRDNames[iZ]))
    end

    ######IO Code for RD
    #only need N as a descriptive row
    descRowNamesRD::Vector{String} = ["N ('000s)"]

    #need to print the descriptive rows
    descContentRD::Vector{Vector{String}} =
        [Vector{String}(length(modelsRD)) for i∈ 1:length(descRowNamesRD)]
    for i ∈ 1:length(XRDSpecs)
        descContentRD[1][i] = "$(round(modelsRD[i].N/1000.0,1))"
    end

    TableTextRD::String = texTable(modelsRD, getHomoskedΣ!, [:intercept; :negLFlows],
        caption = "Performance Flows RD Specifications",
        colNames = [["Thresholds for significant redemptions (\\% of fund)"],
            ["($(Int(collect(SIG_REDEMPTION_THRESHOLDS)[i]*100.0))\\%)" for i∈1:length(modelsRD)]],
        contentRowNames = ["intercept", "outflows/AUM x 100"],
        ...
        columnSepPt = -20,
        scaling = [1.0,1.0],
        widthColNames = [[length(modelsRD)],ones(Int,length(modelsRD))],
        clearMem = USE_AGGRESSIVE_GC)
...
function verify2R(;FlowsLaggingProcedure::Char = 'U')
    ...
    close(iStream)
    ctr = 0.0
#=    for d ∈ unique(HFData[:,:fDate])
        try
            writetable("$R_TEST_PATH\\test.csv",HFData[HFData[:,:fDate].==d,:])
            ctr += 1.0
        catch
            println("Error detected for fData=$d, ctr=$ctr")
            for f ∈  unique(HFData[HFData[:,:fDate].==d,:fund_id])
                ctr+= .0001
                try
                    writetable("$R_TEST_PATH\\test.csv",HFData[(HFData[:,:fDate].==d) & (HFData[:,:fund_id].==f),:])
                catch
                    warn("Could not print $(HFData[(HFData[:,:fDate].==d) & (HFData[:,:fund_id].==f),:]), ctr=$ctr")
                end
                writetable("$R_TEST_PATH\\test.csv",HFData[(HFData[:,:fDate].==d) & (HFData[:,:fund_id].==f),:])
            end
        end
