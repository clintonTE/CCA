...
#  HFData[:,:inception] = ((x::Int)->Date("$x",dtFormat)).(HFData[:,:inception])
 ...
 # HFData[:,:fund_assets_as_of] = ((x::Int)->Date("$x",dtFormat)).(HFData[:,:fund_assets_as_of])
  ...
  by(HFData, :fund_id) do HFSub::SubDataFrame
    ...
    for i::Int ∈ 1:nPer
        if !isna(HFSub[i,:flows]) && i::Int > HFSub[i,:monthsNotice]

            #note notifications of flows does not include the current month's flows
            if HFSub[i,:monthsNotice] ≥ MIN_MONTHS_NOTICE_IV
                HFSub[(i-HFSub[i,:monthsNotice]):(i-1),:rolling_notice] += HFSub[i,:flows]
            end

            if HFSub[i,:flows] < 0.0

                #record the notification on the notification date
                HFSub[i-HFSub[i,:monthsNotice],:redemp_notice] = HFSub[i,:flows]
                HFSub[i-HFSub[i,:monthsNotice],:redemp_notice_dt] = HFSub[i,:date]
  ...
  println("Max Months' Notice: $(unique(HFData[:,:monthsNotice]))")
...
function processLongHFData(;FlowsLaggingProcedure::Char = 'U')
    ...
    HFLong::DataFrame = melt(HFData, [:negLFlows,:monthsNotice, :fDate, :main_strategy])
    ...
    HFLong = HFLong[HFLong[:,:monthsNotice].≥MIN_MONTHS_NOTICE_RD,:]
...
##################IO Code for 2SLS
    ...
    #=HFData[:,:negLFlowsG0] = HFData[:,:negLFlows]
    HFData[:,:performance100] = HFData[:,:performance]*100.0

    #need a version with 0s as NAs
    HFData[~isna(HFData[:,:negLFlowsG0]) & (
        HFData[:,:negLFlowsG0].>0.0),:negLFlowsG0] = NA=#
    ...
    writeTables2File([TableSummary, TableTextLM, TableText1stStage, TableText2SLS, TableTextRD],
...
processLongHFData(FlowsLaggingProcedure=FLPChar)
AnalyzeAggregates(FlowsLaggingProcedure=FLPChar, verbose=false)
