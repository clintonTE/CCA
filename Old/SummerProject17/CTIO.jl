...
using DataFrames, CTReg, Distributions, Formatting

export texTable, writeTables2File, array2String, num2Str, vec2String
...
function num2Str(x::T, decimals::Int=DEFAULT_DECIMALS;
    Ints::Bool = false, scaleHurdle::U=nothing)::String where {T<:Real, U<:Union{Void,Real}}

  if scaleHurdle ≠ nothing && x≥scaleHurdle
    outStr::String = "$(Int(round(x)))"
  elseif Ints && round(x) == x
    outStr = "$(Int(x))"
  else
...
function texTable(models::Vector{CT2SLS}, getΣ::Function, rows::Vector{Symbol};
    ...
    return texTable(titleCaption, caption, colNames, contentRowNames, content, descRowNames,
        descContent, notes, arrayStretch=arrayStretch, lineSpacer=lineSpacer, summaryMathMode=summaryMathMode, widthColNames=widthColNames,
        widthDescContent=widthDescContent, alignmentColNames=alignmentColNames)
end
