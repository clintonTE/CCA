...
using Distributions, StatsBase, DataFrames, NLopt, ForwardDiff

export skewnessStat, kurtosisStat, JBStat, UnivariateTest, autoρ, ljungBox, test, pAutoρ,
  ARMA, ARMAFillYHat!, ARMALogLike, ARMAEstimate, generalizeARMA!, narrowARMA!, lagDF!

importall CTReg, CTIO
...
ljungBox(data::Vector{Float64}, lagRange::Range = 1:(Int(round(log(length(data)))))) =
...
function autoρ(v::T, lag::Int)::Float64 where T <: AbstractVector
  ...
  rng::Range = (lag+1):length(Δv)
  vCov::Float64 = sum(((i::Int)->Δv[i]*Δv[i-lag]).(rng))
  return vCov/sum(Δv.^2.)
end

#convenience function to take in many lags
autoρ(v::T where T <: AbstractVector, lags::Range)::Vector{Float64} = ((l::Int)->autoρ(v,l)).(lags)
...
#convenience function to take in many lags
pAutoρ(v::T where T <: AbstractVector, lags::Range)::Vector{Float64} =
...
function generalizeARMA!(Θ::ARMA)::Void
  Θ.c = Real(Θ.c)
  Θ.ϕ = Vector{Real}(Θ.ϕ)
  Θ.ϑ = Vector{Real}(Θ.ϑ)
  Θ.σ² = Real(Θ.σ²)
...
function narrowARMA!(Θ::ARMA)::Void
  Θ.c = Float64(Θ.c)
...
function ARMAEstimate(Θ::ARMA, Y::Vector{Float64};
  ...
  function update∇!(θ::Vector{Float64}, ∇::Vector{Float64})::Void
    if length(∇) > 0
