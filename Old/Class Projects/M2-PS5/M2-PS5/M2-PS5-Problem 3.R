...
print("2SLS Homoskedastic Standard Error");
print(sqrt(diag(lSLSOut$mHSSE)));

print("2SLS Modified White Standard Error");
print(colnames(lSLSOut$mMWSE));
print(sqrt(diag(lSLSOut$mMWSE)));
...
print("check on OLS");
print(sqrt(diag(vcovHC(vOLS, type = "HC3"))));
print("second check 2SLS");
mZa = cbind(mZ,mW)
mPZa = mZa %*% solve(t(mZa) %*% mZa) %*% t(mZa);
mXt = mPZa %*% cbind(mX, mW);
print(sqrt(diag(GetMWhiteErrors(mXt, resid(aerReg)))));
