...
testDat = data.table(read.csv("RHFData.csv", header = TRUE))
#testDat$fund_id = as.factor(testDat$fund_id)
head(testDat)
...
specLM = as.formula(paste("performance ~ isNoticePeriod+ isRedemptionPeriod + fDate"))
...
print(sqrt(diag(vcovHC(OLS, type = "HC3"))))

print("OLS Modified White Standard Errors, Clustered on fund")
print(sqrt(diag(vcovHC(OLS, type = "HC3", cluster="fDate"))))
